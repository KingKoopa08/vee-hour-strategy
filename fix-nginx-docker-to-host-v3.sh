...
# Fix nginx to connect to host PM2 instead of Docker service (v3 - restart method)
...
# Get gateway IP
echo -e "${YELLOW}Step 1/8: Getting Docker gateway IP...${NC}"
GATEWAY_IP=$(docker exec "$DOCKER_NGINX" ip route | grep default | awk '{print $3}')
echo -e "${GREEN}✅ Gateway IP: $GATEWAY_IP${NC}"
echo ""

# Copy config out
echo -e "${YELLOW}Step 2/8: Extracting config...${NC}"
docker cp "$DOCKER_NGINX:$CONFIG_FILE" /tmp/nginx-default.conf
echo -e "${GREEN}✅ Config extracted${NC}"
echo ""

# Show current config
echo -e "${YELLOW}Step 3/8: Current upstream configuration:${NC}"
grep -A 1 "upstream" /tmp/nginx-default.conf
echo ""

# Backup config
echo -e "${YELLOW}Step 4/8: Creating backup...${NC}"
cp /tmp/nginx-default.conf /tmp/nginx-default.conf.backup.$(date +%Y%m%d-%H%M%S)
echo -e "${GREEN}✅ Backup created${NC}"
echo ""

# Update config
echo -e "${YELLOW}Step 5/8: Updating config to use gateway IP...${NC}"
...
echo -e "${YELLOW}Step 6/8: Stopping nginx to replace config...${NC}"
docker exec "$DOCKER_NGINX" nginx -s stop 2>/dev/null || docker exec "$DOCKER_NGINX" killall nginx 2>/dev/null || true
sleep 2
echo -e "${GREEN}✅ Nginx stopped${NC}"
echo ""

# Copy config back
echo -e "${YELLOW}Step 7/8: Copying updated config...${NC}"
docker cp /tmp/nginx-default.conf "$DOCKER_NGINX:$CONFIG_FILE"
echo -e "${GREEN}✅ Config copied${NC}"
echo ""

# Start nginx
echo -e "${YELLOW}Step 8/8: Starting nginx...${NC}"
docker exec "$DOCKER_NGINX" nginx
sleep 2

if docker exec "$DOCKER_NGINX" nginx -t 2>&1 | grep -q "successful"; then
    echo -e "${GREEN}✅ Nginx started and config valid${NC}"
else
